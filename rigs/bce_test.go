...
func TestBCEBillboard(t *testing.T) {
	...
	// B=... keeps the marker out of the echoed command line, so only the
	// printed bar itself contains the contiguous string BCEBAR
	r.SendKeys(shell, `clear; B=BCE; printf '\033[44m'"$B"'BAR\033[K\033[0m\n'`, "Enter")
	sleep(500)
