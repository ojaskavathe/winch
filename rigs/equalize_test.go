...
// TestEqualize: winch equalize coexistence — equalize while docked,
// geometry-free leave, proportional give-back at release and at undock.
func TestEqualize(t *testing.T) {
	r := New(t)

	mains := func() [][3]string { // [pane, left, width] of w1's non-winch panes
		var out [][3]string
	...
	r.Chk("dirty marker set", r.ShowOpt("-wv", "-t", r.W1, "@winch_layout_dirty") == "1")
	...
	r.Chk("dirty marker cleared", r.ShowOpt("-wv", "-t", r.W1, "@winch_layout_dirty") == "")
	...
	r.Chk("no sidebar in w1", r.WinchPanes("-t", r.W1) == 0)
	r.Chk("dirty cleared on undock", r.ShowOpt("-wv", "-t", r.W1, "@winch_layout_dirty") == "")
}
